...
export function overrideInternalLinks(container: HTMLElement, hostname: string) {
 ...
 */
export function createLinkObserver(callback: (el: HTMLAnchorElement) => void): MutationObserver {
  return new MutationObserver((mutations) => {
    for (const node of mutations.flatMap((m) => Array.from(m.addedNodes))) {
      ...
      if (el.tagName === "A" && el.hasAttribute("href") && !el.dataset.preserveUrl) {
